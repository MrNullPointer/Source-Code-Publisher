...
const std::string saveFilePath = "..\\DownloadedPages";
const std::string sendFilePath = "..\\ConvertedPages";
...
bool Sender::sendFile(Message msg)
{
  if (!msg.containsKey("file"))
    return false;
  std::string fileSpec = msg.file();//sendFilePath + "/" + msg.file();
...
public:
  ...
  */
  bool receiveFile(Message msg)
  {
	...
	fileName = fileName.substr(location, fileName.length());
